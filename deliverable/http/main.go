...
func init() {
	...
	pqconncmp, err = sql.Open("postgres", "user=postgres password=akuadane database=ijobs sslmode=disable")
...
func main() {
	...
	// Data Repositories
	applicationRepo := apprepo.NewAppRepo(pqconncmp)
	companyRepo := cmprepo.NewCompanyGormRepositoryImpl(gormDB)
	jobRepo := jobrepo.NewJobRepository(pqconncmp)
	categoryRepo := jobrepo.NewCategoryRepositoryImpl(pqconncmp)
